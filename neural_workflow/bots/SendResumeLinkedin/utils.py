...
def getUrlDataFile():
    ...
    try:
        ...
        text = "FileNotFound: o arquivo urlData.txt não foi encontrado. Por favor, execute ./data pasta existe e verifique seus valores config.py. Em seguida, execute o bot novamente"
        prRed(text)
    return urlData
...
def writeResults(text: str):
    ...
    fileName = "Dados de Vagas Aplicadas - " +timeStr + ".txt"
    try:
        ...
        with open("data/" +fileName, 'w' ,encoding="utf-8") as f:
            f.write("---- Dados de empregos aplicados ---- criados em: " +timeStr+ "\n" )
            f.write("---- Número | Cargo | Empresa | Localização | Local de trabalho | Data de postagem | Aplicações | Resultado "   +"\n" )
            for line in lines: 
                f.write(line)
            f.write(text+ "\n--")
        ...
        with open("data/" +fileName, 'w', encoding="utf-8") as f:
            f.write("---- Dados de empregos aplicados ---- criados em: " +timeStr+ "\n" )
            f.write("\n---- Número | Cargo | Empresa | Localização | Local de trabalho | Data de postagem | Aplicações | Resultado " +"\n" )

            f.write(text+ "\n--")

def printInfoMes(bot:str):
    prYellow("ℹ️ " +bot+ " está começando em breve... ")

def finish(self):
    prYellow('Você gostou do projeto, por favor nos apoie com qualquer valor...')
    try:
        self.driver.get('Chave PIX: fabricio.albergaria@gmail.com 😊🤖')
